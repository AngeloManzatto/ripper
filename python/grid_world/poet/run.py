...
def mutate_envs(
    ...
    for parent in EA_list:
        ...
        results = evaluate_match(agents, agent_ids, world, eval_episodes, verbose)
...
def best_transfer_candidate(EA_list, m, entry, world, eval_episodes, role, verbose=False):
    """
    ...
    for other_m, other in enumerate(EA_list):
        ...
        results = evaluate_match(candidate_agents, candidate_agent_ids, world, eval_episodes, verbose=verbose)
...
def attempt_transfer(EA_list, eval_episodes=30, verbose=False):
    """
    ...
    for m, entry in enumerate(EA_list):
        ...
        best_player = best_transfer_candidate(EA_list, m, entry, world, eval_episodes, role="player", verbose=verbose)
        ...
        best_enemy = best_transfer_candidate(EA_list, m, entry, world, eval_episodes, role="enemy", verbose=verbose)
...
def format_results(results, player_id):
    """One readable line per agent: role, win/timeout rates, outcome breakdown."""
    lines = []
    for agent_id, stats in results.items():
        role = "player" if agent_id == player_id else "enemy "
        breakdown = ", ".join(
            f"{reason.split('.')[-1]}={count}"
            for reason, count in stats["breakdown"].items()
        )
        lines.append(
            f"    {role}(id={agent_id}) win={stats['win_rate']:.0%} "
            f"timeout={stats['timeout_rate']:.0%} | {breakdown}"
        )
    return "\n".join(lines)
...
def run_poet(
    ...
    for t in range(T):
        ...
        for e in EA_list:
            ...
            results = evaluate_match(agents, agent_ids, world, eval_episodes, verbose=verbose)
            e["last_eval"] = results
        ...
        if len(EA_list) > 1 and t % N_transfer == 0:
            EA_list = attempt_transfer(EA_list, eval_episodes=eval_episodes, verbose=verbose)
        ...
        for i, e in enumerate(EA_list):
            print(f"  entry {i}:")
            print(format_results(e["last_eval"], e["player_id"]))
